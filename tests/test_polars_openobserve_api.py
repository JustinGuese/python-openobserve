"""
Pytest file for python-openobserve with polars
...
def test_search1_df():
    ...
    start_timeperiod = datetime.now() - timedelta(days=7)
    end_timeperiod = datetime.now()
    df_search_results = oo_conn.search2df_polars(
        sql,
    ...
    assert not df_search_results.is_empty()
...
def test_search1_dftypes():
    ...
    assert not df_search_results.is_empty()
    assert df_search_results.shape
    assert df_search_results["_timestamp"].dtype == polars.Int64
    ...
    assert not df_search_results.is_empty()
    assert df_search_results.shape
    assert df_search_results["_timestamp"].dtype == polars.Datetime(
