...
class OpenObserve:
    def __init__(self, user: str, password: str, organisation: str = "default", 
                 host: str = "http://localhost:5080", verify: bool = True) -> None:
        bas64encoded_creds = base64.b64encode(f"{user}:{password}".encode("utf-8")).decode("utf-8")
        self.openobserve_url = f"{host}/api/{organisation}/[STREAM]"
        self.openobserve_host = host
        self.headers = {'Content-Type': 'application/json', "Authorization": f"Basic {bas64encoded_creds}"}
        self.verify = verify
    ...
    def _handle_response(self, res: requests.Response, action: str = "request") -> dict:
        ...
        if res.status_code != requests.codes.ok:
            raise Exception(f"Openobserve {action} returned {res.status_code}. Text: {res.text}")
        return res.json()
    ...
    def index(self, index: str, document: dict) -> dict:
        ...
        res = requests.post(
            ...
            headers=self.headers, json=[document], verify=self.verify
        )
        response_json = self._handle_response(res, "index")
        
        if response_json["status"][0]["failed"] > 0:
            raise Exception(f"Openobserve index failed. {response_json['status'][0]['error']}. document: {document}")
        return response_json

    def search(self, sql: str, start_time: Union[datetime, int] = 0, 
               end_time: Union[datetime, int] = 0, verbosity: int = 0, 
               outformat: str = 'json') -> List[Dict]:
        ...
        res = requests.post(
            ...
            json=query, headers=self.headers, verify=self.verify
        )
    ...
    def _execute_api_request(self, endpoint: str, verbosity: int = 0, 
        ...
        if method == "GET":
            res = requests.get(url, headers=self.headers, params=params, verify=self.verify)
        elif method == "POST":
            res = requests.post(url, headers=self.headers, json=json_data, verify=self.verify)
        elif method == "PUT":
            res = requests.put(url, headers=self.headers, json=json_data, verify=self.verify)
        else:
    ...
    def list_streams(self, schema: bool = False, streamtype: str = 'logs', 
                     verbosity: int = 0, outformat: str = 'json'):
        """
    ...
    def config_export(self, file_path: str, verbosity: int = 0, outformat: str = 'json'):
        ...
        # Collect all data
        data = {name: self.list_objects(api_path, verbosity=verbosity, outformat=data_format) 
    ...
    def import_functions(self, file_path: str, overwrite: bool = False, verbosity: int = 0):
    ...
    def config_import(self, object_type: str, file_path: str, overwrite: bool = False, verbosity: int = 0):
        ...
        if object_type == 'all':
            importable_types = [
                'functions', 'pipelines', 'alerts', 
                'alerts/destinations', 'alerts/templates', 'dashboards'
                # 'streams' and 'users' are not supported by the API
            ]
            
            for item in importable_types:
                file_suffix = item.replace('/', '-')
                self.import_objects(item, f"{file_path}{file_suffix}.json", overwrite, verbosity)
        else:
